...
可放入 prompt 的關鍵詞，全部從軟體工程、邏輯學、認知科學、語言學、品質保證五大領域借用而來。這本辭典不發明新詞、不收錄 prompt engineering 內部黑話（few-shot、RAG 等），只挑「已經在其他領域有明確定義、可以直接搬進 prompt 當控制詞使用」的術語。每個詞條給四個欄位：來源、本義、Prompt 用法、範例，方便 Ctrl-F 翻完直接抄進對話框。
